...
pub fn perform_valid_2d_conv_with_boundary(fir_filter: &Array2<f64>, input_matrix: &mut Array2<f64>) -> Array2::<f64>
{
...
pub fn add_matrix_boundary(input_matrix: &mut Array2<f64>) -> Array2::<f64>
{
    ...
    for i in 0..output_matrix.ncols()
    {
        ...
        output_matrix.row_mut(output_matrix.nrows() - 1)[i] = output_matrix.row(output_matrix.nrows() - 2)[i];
    }
    ...
    for i in 0..output_matrix.nrows()
    {
        ...
        output_matrix.column_mut(output_matrix.ncols() - 1)[i] = output_matrix.column(output_matrix.ncols() - 2)[i];
    }
    output_matrix
}

pub fn copy_matrix_within_padding(input_matrix: &Array2<f64>,
...
{
    let mut output_matrix = Array2::<f64>::zeros((row_prepad_amount + input_matrix.nrows() + row_postpad_amount, col_prepad_amount + input_matrix.ncols() + col_postpad_amount));

    for row_i in 0..input_matrix.nrows()
    {
        for col_i in 0..input_matrix.ncols()
        {
            output_matrix[(row_i + row_prepad_amount, col_i + col_prepad_amount)] = input_matrix[(row_i, col_i)];
        }   
    }
    output_matrix
}