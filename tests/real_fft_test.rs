...
use visqol_rs::{misc_audio::load_as_mono, misc_audio::mirror_spectrum, misc_math::next_pow_two};
...
fn small_test() {
    ...
    c2r.process(&mut out[0..length / 2 + 1], &mut inverse_signal)
        .unwrap();

    inverse_signal
        .iter_mut()
        .for_each(|x| *x *= 1.0 / length as f64);
