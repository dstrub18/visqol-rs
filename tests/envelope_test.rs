...
fn test_hilbert_on_signal()
{
    ...
    assert_abs_diff_eq!(result[(0,0)].re,  0.0003036616911888330, epsilon=0.0001);
...
fn test_envelope_on_signal()
{
    ...
    assert_abs_diff_eq!(result[(0,0)], 0.00030159861338215923, epsilon=0.0001);
}
...
fn test_xcorr_pointwise_prod_on_signal()
{
    ...
    let ref_signal_vec = ref_signal.data_matrix.remove_axis(Axis(1)).to_vec();
    ...
    let deg_signal_vec = deg_signal.data_matrix.remove_axis(Axis(1)).to_vec();
    ...
    assert_abs_diff_eq!(result[(0,0)].re, 0.012231532484292984, epsilon=0.001);
}
