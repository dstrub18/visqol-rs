...
pub fn compare_real_matrix(matrix_a: &Array2<f32>, matrix_b: &Array2<f32>, tolerance: f32) {
...
pub fn compare_complex_matrix(
    matrix_a: &Array2<Complex<f32>>,
    matrix_b: &Array2<Complex<f32>>,
    tolerance: f32,
) {
    compare_matrix_dimensions(matrix_a, matrix_b);
    for (elements_a, elements_b) in matrix_a.iter().zip(matrix_b) {
...
pub fn compare_complex_vec(vec_a: &[Complex<f32>], vec_b: &[Complex<f32>], tolerance: f32) {
...
pub fn compare_real_vec(vec_a: &[f32], vec_b: &[f32], tolerance: f32) {
    assert_eq!(vec_a.len(), vec_b.len());
    vec_a.iter().zip(vec_b).for_each(|(a, b)| {
...
mod tests {
    ...
    #[test]
    #[should_panic]
    fn different_matrix_dimensions_cannot_be_compared() {
        let a = Array2::<f32>::zeros((2, 2));
        let b = Array2::<f32>::zeros((4, 1));
    ...
    #[test]
    fn compare_identical_elements_complex() {
        let a = Array2::<Complex<f32>>::zeros((2, 2));
        let b = Array2::<Complex<f32>>::zeros((2, 2));
